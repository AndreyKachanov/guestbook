...
class M_Validation
{
	...
	public function execute($obj, $pk = null)
	{
			...
			if (in_array($key, $this->rules['fields'])) { //если dt входит в массив полей
				...
				if(array_key_exists('not_empty', $this->rules) && in_array($key, $this->rules['not_empty'])){ //если dt есть в массиве not_empty
					...
					if($value == ''){
						...
						elseif(array_key_exists('email', $this->rules) && in_array($key, $this->rules['email']) && !filter_var($value, FILTER_VALIDATE_EMAIL)){
		...
		if (!empty($obj['url'])) {
			if(!filter_var($obj['url'], FILTER_VALIDATE_URL)){
				
				$this->errors[] = array('url', 'url');
			}					
		}
